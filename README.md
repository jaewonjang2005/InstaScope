...
> **"당신이 가장 끌리는 해시태그와 숨겨진 취향은 무엇인가요?"**  
...
- 인스타그램에 남긴 활동 내역을 텍스트 기반(해시태그, 카테고리 라벨)으로 파싱하여, 가장 많이, 가장 깊게 반응한 핵심 키워드(일반/숨겨진 취향)를 도출합니다.

### 🛡️ 2. 안전한 우회 스크래핑 기반 콘텐츠 추천 (Vector DB ❌ -> Real-time Search ⭕)
- **민감한 취향 단어 필터링 및 우회 매핑**: 검색 엔진에서 차단될 수 있는 일부 민감한 단어들을 안전한 대체 단어로 자동 치환(Safe Mapping)하여 원활하고 끊김 없는 추천 검색을 수행합니다.
