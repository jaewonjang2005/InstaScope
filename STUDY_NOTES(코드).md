...
## 6. AI와 함께 깊게 공부하는 법 — 이 프로젝트로 실제 검증된 방법

### 왜 이게 문제였는가
스터디를 준비하면서 재원님이 마주친 진짜 문제는 "코딩을 못한다"가 아니라, **"AI에게 위임하면서도 어떻게 실질적으로 공부할 것인가"**였습니다. 재원님의 충만주의는 결과보다 과정 몰입을 중요하게 여기는데, 2주짜리 토이 프로젝트에서 모든 코드를 손으로 타이핑하며 과정을 밟을 시간은 없었죠. 그런데 위의 1~5번을 만드는 과정 자체가 이미 답을 보여줬습니다: **"쓰면서 배우기"가 아니라 "뜯어보면서 배우기"도 똑같이 깊은 과정 몰입**이라는 것입니다. 코드를 짜는 순간에 몰입하지 못했다면, 짜여진 결과를 형사처럼 추적하는 순간에 몰입하면 됩니다. 방향이 다를 뿐, 충만주의가 요구하는 "깊이"는 그대로 만족됩니다.

### 실제로 검증된 5가지 방법 (전부 이 프로젝트에서 방금 일어난 일)

**① 문서와 실제 코드를 대조해서 검증하는 습관**
가장 결정적인 발견이 이거였죠 — README·docs에는 "DuckDuckGo로 검색한다"고 적혀 있었지만, 실제 `search_service.py`를 열어보니 전혀 다른 방식(인스타그램 explore 링크 생성)으로 동작하고 있었습니다. AI가 짜준 코드, 혹은 AI가 짜준 문서를 그대로 믿는 순간 공부는 멈춥니다. **"문서가 뭐라고 하든, 실제로 실행되는 코드가 뭘 하는지"를 항상 별도로 확인하는 습관**이 진짜 이해의 출발점입니다. 다음 프로젝트에서도 완성됐다고 넘어가기 전에 "내가 알던 설계와 실제 코드가 일치하는지" 한 번은 꼭 대조해보세요.

**② "왜 이렇게 짰지?"를 스스로 먼저 추측해보고 답과 비교하기**
5번 섹션(기술 스택 선택 이유)을 그냥 읽고 "아 그렇구나" 하고 넘기면 정보 소비에 그칩니다. 대신 각 항목을 보기 전에 "내가 Vercel 서버리스의 500MB/60초 제약을 몰랐다면 나는 어떤 선택을 했을까?"를 먼저 스스로 답해본 다음 문서와 비교해보세요. 예측이 빗나간 지점이 바로 재원님이 진짜로 몰랐던 지점이고, 그게 공부해야 할 부분입니다. Vector DB(pgvector/FAISS)를 쓰려다 메모리 문제로 포기하고 즉석 통계 분석으로 전환했다는 대목이 특히 좋은 연습 재료예요 — 왜 그 선택이 "타협"이 아니라 "제약 안에서의 최선"인지 스스로 설명해보세요.

**③ 예시를 하나 더 만들어서 손으로 계산해보기**
3번 섹션의 헬스+집밥 예시를 읽고 이해했다고 착각하기 쉽지만, 진짜 이해했는지는 **다른 조합을 스스로 만들어서 계산해봐야** 드러납니다. 예를 들어 "좋아요 위주로만 활동한 사용자"나 "한 게시물에 해시태그를 20개 붙인 경우(sibling penalty 발동)"를 가정하고, TF-IDF 점수와 hidden 후보 판별까지 손으로 따라가 보세요. 계산이 문서의 로직과 안 맞으면, 그 지점이 정확히 이해가 빵꾸난 곳입니다.

**④ 죽은 코드를 "실패"가 아니라 "의사결정의 역사"로 재구성하기**
`one_pick.py`, `TopPickCard.jsx`, `chart.js`, `apscheduler`처럼 안 쓰이는 코드들이 남아있죠. 이걸 부끄러운 흔적으로 숨기지 말고, "왜 시작했다가 왜 멈췄는가"로 스토리를 재구성해보세요. 이 습관은 나중에 대학원이나 실무에서 아키텍처 결정 기록(ADR, Architecture Decision Record)을 남기는 습관으로 자연스럽게 이어집니다 — 큰 개인 프로젝트를 할 때 작동원리를 더 잘 파악하고 싶다는 게 애초에 이 스터디를 시작한 이유였으니, 이 습관 하나만 챙겨도 그 목적은 이미 달성되는 셈입니다.

**⑤ 이해한 지점에서 다음 질문으로 체인을 이어가기**
스터디 준비를 시작할 때 "왜 스터디를 하는가"부터 시작해서 제1신념까지 파고 내려간 방식, 그대로 코드 이해에도 적용됩니다. 예: TF-IDF를 이해했으면 → "TF-IDF의 한계는 뭐지?" → "임베딩 기반 유사도랑 뭐가 다르지?" → "이 프로젝트에 임베딩을 쓰려면 어떤 제약을 넘어야 하지?"(문서에 나온 pgvector 메모리 폭발 얘기와 바로 연결됩니다). 질문 하나를 답으로 끝내지 않고 다음 질문을 만들어내는 게, AI에게 위임한 코드에서도 "진짜 내 이해"를 만들어내는 방법입니다.

### 다음 프로젝트로 가져갈 것
이 5가지는 이번 프로젝트 한정이 아니라, 앞으로 데이터사이언티스트·AX 방향으로 큰 프로젝트를 할 때마다 반복 가능한 루틴입니다. 특히 프로젝트가 끝날 때마다 지금처럼 Claude Code한테 "문서-코드 대조 + 죽은 코드 찾기 + 예시 트레이스"를 감사(audit)하듯 한 번 돌리는 것 자체를 습관으로 정착시키면, AI에게 구현을 위임하더라도 "내가 뭘 만들었는지 설명 못 하는" 상태는 피할 수 있습니다.
