...
export default function SecretPickPage() {
  ...
  const currentKeywords = isSpicyMode && raw_hidden_tags?.length > 0 ? raw_hidden_tags : hidden_keywords;
  const currentRecommendations = isSpicyMode && spicy_recommendations?.length > 0 ? spicy_recommendations : hidden_recommendations;
  ...
  return (
    <div className="fade-in" style={{ maxWidth: '800px', margin: '0 auto', paddingTop: '2rem' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem', padding: '2rem', background: isSpicyMode ? 'rgba(255,0,0,0.1)' : 'rgba(255,107,107,0.05)', borderRadius: '24px', border: isSpicyMode ? '1px solid rgba(255,0,0,0.4)' : '1px solid rgba(255,107,107,0.2)' }}>
        ...
        <h1 style={{ fontSize: '2.5rem', fontWeight: 800, background: isSpicyMode ? 'linear-gradient(to right, #ff0000, #cc0000)' : 'linear-gradient(to right, #ff6b6b, #ff8e53)', WebkitBackgroundClip: 'text', color: 'transparent', margin: 0 }}>
